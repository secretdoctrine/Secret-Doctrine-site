  ...
  module Pages
    class MenuPresenter
      ...
      config_accessor :roots, :menu_tag, :list_tag, :list_item_tag, :css, :dom_id,
                      ...
                      :link_tag_css
      self.dom_id = 'mainmenu'
      self.css = ''
      self.menu_tag = :table
      self.list_tag = :tr
      self.list_item_tag = :td
      ...
      def render_menu_item(menu_item, index)
        content_tag(list_item_tag, :class => menu_item_css(menu_item, index)) do
          ...
          buffer << render_inner_menu_items(menu_item_children(menu_item))
          buffer
        end
      end

      def render_inner_menu_items(menu_items)
        if menu_items.present?
          content_tag(:ul, :class => list_tag_css) do
            menu_items.each_with_index.inject(ActiveSupport::SafeBuffer.new) do |buffer, (item, index)|
              buffer << render_inner_menu_item(item, index)
            end
          end
        end
      end

      def render_inner_menu_item_link(menu_item)
        link_to(menu_item.title, context.refinery.url_for(menu_item.url))
      end

      def render_inner_menu_item(menu_item, index)
        content_tag(:li, :class => menu_item_css(menu_item, index)) do
          buffer = ActiveSupport::SafeBuffer.new
          buffer << render_inner_menu_item_link(menu_item)
          buffer << render_inner_menu_items(menu_item_children(menu_item))
          buffer
        end
      end
